...
$effect(() => { if (formInput.validate.ctr > 0) doValidate(); });
...
<script>
    ...
    let regions = $derived(
        allRegions.filter(r => r.countryCode === $form.country)
    );
</script>

<Form.Field name="country">
    <Combobox items={countries} bind:value={$form.country}
        onValueChange={() => { $form.region = ""; }} />
</Form.Field>

<Form.Field name="region">
    <Combobox items={regions} bind:value={$form.region} />
</Form.Field>
```
...
it('shows loading state when clicked', async () => {
    render(Button, { label: 'Submit', onClick: async () => { await delay(100); } });
