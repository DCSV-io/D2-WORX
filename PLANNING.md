...
| ------ | ---------------------------------------------------- | -------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ~~32~~ | ~~Thundering herd protection on popular key expiry~~ | Shared   | ✅ Done | `Singleflight` utility added to both .NET (`D2.Shared.Utilities.Singleflight`) and Node.js (`@d2/utilities`). Wired into FindWhoIs on both platforms — wraps circuit breaker call, deduplicating concurrent gRPC requests for the same cache key. First caller executes, others piggyback on the same Promise/Task.       |
| 40     | OTel alerting for service outages                    | All      | Medium | Add OTel-based alerting rules for service unavailability (gRPC failures, RabbitMQ down, Redis down). Overlaps with blocked #7 — combine once AlertManager/Grafana alerting is wired. Define alert rules for: error rate spikes (>5% 5xx over 5min), latency P99 >2s, rate limit blocks per dimension, delivery failures. |
| ~~41~~ | ~~Node.js pre-auth handlers: early DI scope~~        | Shared   | ✅ Done | Resolved via AsyncLocalStorage ambient context. `HandlerContext` checks `requestContextStorage`/`requestLoggerStorage` first — all handlers (including pre-auth singletons) automatically see per-request context. `ambient-scope` middleware wraps pipeline in `.run()`, scope middleware upgrades via `.enterWith()`.     |
| ~~42~~ | ~~CI: auth-bff-client tests missing from workflow~~  | CI       | ✅ Done | Added `node-bff-client-unit` job to `.github/workflows/test.yml`.                                                                                                                                                                                                                                                         |
| ~~43~~ | ~~Missing tests: reset-password-schema~~             | Frontend | ✅ Done | 21 tests added at `clients/web/src/lib/shared/forms/__tests__/reset-password-schema.test.ts`.                                                                                                                                                                                                                             |
| ~~44~~ | ~~Missing tests: forgot-password-schema~~            | Frontend | ✅ Done | 14 tests added at `clients/web/src/lib/shared/forms/__tests__/forgot-password-schema.test.ts`.                                                                                                                                                                                                                            |
| ~~45~~ | ~~Missing tests: form-actions.server.ts~~            | Frontend | ✅ Done | 9 tests added at `clients/web/src/lib/server/form-actions.server.test.ts`. Mocks Superforms + D2Result.                                                                                                                                                                                                                  |
| ~~46~~ | ~~Missing tests: auth-gateway-client~~               | Frontend | ✅ Done | 24 tests added at `clients/web/src/lib/client/rest/auth-gateway-client.test.ts`. Mocks `executeFetch`.                                                                                                                                                                                                                   |
| ~~47~~ | ~~Missing tests: auth.server.ts lazy singleton~~     | Frontend | ✅ Done | 4 tests added at `clients/web/src/lib/server/auth.server.test.ts`. Tests env validation, singleton caching, config passthrough.                                                                                                                                                                                           |
| ~~48~~ | ~~CLAUDE.md: wrong gateway-client path~~             | Docs     | ✅ Done | Fixed `$lib/shared/rest/` → `$lib/client/rest/` in CLAUDE.md.                                                                                                                                                                                                                                                             |
| ~~49~~ | ~~Duplicate signup schema in design route~~          | Frontend | ✅ Done | Deleted `routes/design/signup-form/schema.ts`, updated `+page.server.ts`, `+page.svelte`, and test to import from `$lib/shared/forms/sign-up-schema.js`.                                                                                                                                                                 |
| ~~50~~ | ~~Contact schema lives in design route, not shared~~ | Frontend | ✅ Done | Moved `createContactSchema` to `$lib/shared/forms/contact-schema.ts`, updated `contact-form/+page.server.ts`, `+page.svelte`, and `contact-schema.test.ts` imports.                                                                                                                                                      |
| ~~51~~ | ~~Triplicated fetch pattern in 3 gateway clients~~   | Frontend | ✅ Done | Extracted `executeFetch()` to `$lib/shared/rest/gateway-response.ts`. All 3 clients (`gateway.server.ts`, `gateway-client.ts`, `auth-gateway-client.ts`) now delegate to it.                                                                                                                                              |
| ~~52~~ | ~~.NET Gateway CORS: single origin only~~            | Gateway  | ✅ Done | `Program.cs` now splits `CorsOrigin` on `,` and spreads into `WithOrigins(...)`.                                                                                                                                                                                                                                          |
| ~~53~~ | ~~PLANNING.md test counts are stale~~                | Docs     | ✅ Done | Updated: Geo 798, Auth 972, Comms 597, Shared 1094, BFF 42, dkron-mgr 64, Web 525, E2E 20+2 (invitation flaky).                                                                                                                                                                                                         |
...
| ~~6~~ | ~~Validate PII redaction in OTel output~~ | ✅ Done                                        | ✅ Done  | Verified: no IPs in spans/logs, fingerprints + WhoIs IDs are hashed, UAs redacted by handler RedactionSpec                                                     |
| 7     | OTel alerting rules                       | ~~Running AlertManager/Grafana~~ **UNBLOCKED** | Medium   | Grafana is running. Overlaps with open #40. Define: error rate >5% 5xx over 5min, latency P99 >2s, rate limit blocks per dimension, delivery failures, gRPC failure rate. Requires Grafana alerting channel config + alert rule YAML. Can start now — not blocked. |
...
- @d2/auth-bff-client package (ADR-017) — 34 unit + 10 E2E tests (per-session JWT cache fix)
