...
Shared configuration providing observability infrastructure (OpenTelemetry, Serilog), health checks, and resilience patterns. Traces and logs route through Alloy via OTLP; .NET metrics are scraped by Alloy's Prometheus endpoint. All signals land in the LGTM stack (Loki, Grafana, Tempo, Mimir).
...
| [Extensions.IHostApplicationBuilder.cs](Extensions.IHostApplicationBuilder.cs)       | Internal implementation. `AddServiceDefaults()` calls `D2Env.Load()` + `AddEnvironmentVariables()` to pick up `.env` transforms, then configures Serilog (console), OpenTelemetry tracing/metrics (ASP.NET Core, gRPC, HTTP client, runtime, process), Prometheus exporter, OTLP exporters (traces + logs via Alloy), and health checks. Filters out infrastructure endpoints (/health, /alive, /metrics) from traces and logs. |
