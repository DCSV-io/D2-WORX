  ...
  async handleAsync(
    ...
    return BaseHandler.tracer.startActiveSpan(handlerName, async (span) => {
      ...
      try {
        ...
        // Log input as debug if enabled
        if (opts.logInput) {
          this.context.logger.debug(
            `Handler ${handlerName} received input: ${JSON.stringify(input)}. TraceId: ${this.context.request.traceId}.`,
          );
        }
        ...
        // Log output as debug if enabled
        if (opts.logOutput) {
          this.context.logger.debug(
            `Handler ${handlerName} produced result: ${JSON.stringify(result)}. TraceId: ${this.context.request.traceId}.`,
          );
        }
