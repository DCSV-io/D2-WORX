...
import { RunDeliveryHistoryPurge, DEFAULT_COMMS_JOB_OPTIONS } from "@d2/comms-app";
import type { IPurgeDeliveryHistoryHandler } from "@d2/comms-app";
...
function createMockPurge(rowsAffected = 0) {
  ...
  } as unknown as IPurgeDeliveryHistoryHandler;
}

describe("RunDeliveryHistoryPurge", () => {
  ...
  it("should purge with correct cutoff date based on deliveryHistoryRetentionDays", async () => {
    const before = new Date();
    before.setDate(before.getDate() - DEFAULT_COMMS_JOB_OPTIONS.deliveryHistoryRetentionDays);
  ...
  it("should return correct rowsAffected", async () => {
    ...
    expect(result.success).toBe(true);
    expect(result.data?.rowsAffected).toBe(200);
  ...
  it("should use custom retention days from options", async () => {
    const customOptions = { ...DEFAULT_COMMS_JOB_OPTIONS, deliveryHistoryRetentionDays: 30 };
    handler = new RunDeliveryHistoryPurge(
      acquireLock,
      releaseLock,
      purge,
