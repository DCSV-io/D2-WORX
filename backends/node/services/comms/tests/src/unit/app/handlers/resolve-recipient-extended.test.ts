...
describe("RecipientResolver — extended coverage", () => {
  it("should resolve by contactId using GetContactsByIds", async () => {
    const contacts = new Map();
    contacts.set("contact-42", {
      id: "c1",
    ...
    expect(geoIds.handleAsync).toHaveBeenCalledWith({
      ids: ["contact-42"],
    });
  });
  ...
  it("should resolve phone-only when email not available", async () => {
    const contacts = new Map();
    contacts.set("auth_user:user-phone", [
      {
        id: "c3",
        contextKey: "auth_user",
  ...
  it("should prefer userId over contactId when both provided", async () => {
    ...
    expect(geo.handleAsync).toHaveBeenCalledWith({
      keys: [{ contextKey: "auth_user", relatedEntityId: "user-both" }],
    });
  });
