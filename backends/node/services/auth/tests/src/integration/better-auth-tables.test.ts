...
describe("BetterAuth tables (integration)", () => {
      ...
      user: {
        create: {
          before: async (user: Record<string, unknown>) => {
            return { data: ensureUsername(user) };
          },
        },
      },
    },
  ...
  describe("User + Account", () => {
    ...
    it("should populate all user columns in database", async () => {
      ...
      expect(row.name).toBe("Field Test");

      // Username fields — auto-generated by ensureUsername hook
      ...
      expect(typeof row.display_username).toBe("string");
      expect(row.display_username.length).toBeGreaterThan(0);
  ...
  describe("Session", () => {
    it("should create a session record on sign-up", async () => {
      ...
      const result = await getPool().query(`SELECT * FROM session WHERE user_id = $1`, [user.id]);
  ...
  describe("Organization", () => {
    ...
    it("should create a member linking user to org", async () => {
      ...
      const result = await getPool().query(`SELECT * FROM member WHERE organization_id = $1`, [
        org.id,
      ]);
      expect(result.rows.length).toBeGreaterThanOrEqual(1);
      expect(result.rows[0].user_id).toBe(user.id);
