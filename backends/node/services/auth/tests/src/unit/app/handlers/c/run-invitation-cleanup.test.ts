...
import { RunInvitationCleanup, DEFAULT_AUTH_JOB_OPTIONS } from "@d2/auth-app";
import type { IPurgeExpiredInvitationsHandler } from "@d2/auth-app";
...
function createMockPurge(rowsAffected = 0) {
  ...
  } as unknown as IPurgeExpiredInvitationsHandler;
}
...
describe("RunInvitationCleanup", () => {
  ...
  it("should purge with correct cutoff date based on invitationRetentionDays", async () => {
    const before = new Date();
    before.setDate(before.getDate() - DEFAULT_AUTH_JOB_OPTIONS.invitationRetentionDays);
  ...
  it("should return correct rowsAffected", async () => {
    ...
    expect(result.success).toBe(true);
    expect(result.data?.rowsAffected).toBe(25);
  ...
  it("should use custom retention days from options", async () => {
    const customOptions = { ...DEFAULT_AUTH_JOB_OPTIONS, invitationRetentionDays: 14 };
    handler = new RunInvitationCleanup(
      acquireLock,
      releaseLock,
      purge,
